...
for WEBSERVICE in $WEBSERVICES; do
	...
	PROD_INSIDE_FILE_SOURCE_HOSTNAME='tx-websvc01'
	PROD_DMZ_FILE_SOURCE_HOSTNAME='tx-websvc04'
	...
	elif ssh $PROD_INSIDE_FILE_SOURCE ls -d $PROD_INSIDE_WS_ROOT/$WEBSERVICE/$PROD_INSIDE_FILE_SOURCE_HOSTNAME > /dev/null; then 
	...
	elif ssh $PROD_INSIDE_FILE_SOURCE ls -d $PROD_INSIDE_WS_ROOT/$WEBSERVICE/$PROD_DMZ_FILE_SOURCE_HOSTNAME > /dev/null; then 
		rsync -ave ssh --exclude=logs/* --exclude=temp/* --exclude=work/* \
